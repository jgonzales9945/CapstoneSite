...
<html>
    <head>
        <meta charset="UTF-8">
        <title>Organizations Dashboard - Capstone Site</title>
    ...
    <body>
        <?php
            ...
            if($conn != NULL) {
				...
				try {
					...
					$sql = "SELECT * FROM organizations ORDER BY id DESC LIMIT 15 OFFSET ". ($page * 15) ."";
					...
					if($rowcount > 0) {
						...
						while($row = $result->fetch_assoc()) {
							...
							echo "<tr>";
                            echo "<td><a id='link_title' href='dashboardshow.php?table=". urlencode("organizations") ."&id=". urlencode($row['id']) ."'>". $row['name'] ."</a></td>";
							echo "<td>". $row['location'] ."</td>";
							//echo "<td>". $row['end_date'] ."</td>";
							//check admin
            				if(isset($_SESSION['login_user']) && $_SESSION['login_user'] != '') {
								echo "<td><a href='dashboardedit.php?table=". urlencode("organizations") ."&id=". urlencode($row['id']) ."'>Modify</a></td>";
            		...
            		if(isset($_SESSION['login_user']) && $_SESSION['login_user'] != '') {
    	    			echo "<div><a href='dashboardadd.php?table=organizations'>Add to the database</a></div>";
					}
					if($page > 1 && ($maxid % 15) == 0) { echo "<a id='previous_page' href='dashboardorganizations.php?page=". urlencode($page - 1) .">Previous Page</a>";}
					if($page >= 1) { echo "<a id='next_page' href='dashboardorganizations.php?page=". urlencode($page + 1) .">Next Page</a>";}
					disconnect($conn);
		...
		<script type='text/javascript'>
			function delete_entry(id) {
				...
				if (answer) {
					window.location = 'dashboarddelete.php?table=organizations &id=' + id;
				}
			}
