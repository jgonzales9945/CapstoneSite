...
<html>
    <head>
        <meta charset="UTF-8">
        <title>Scholarships Dashboard - Capstone Site</title>
    ...
    <body>
        <?php
        	...
        	if($conn != NULL) {
				...
				try {
					...
					$sql = "SELECT * FROM scholarships ORDER BY id DESC LIMIT 15 OFFSET ". ($page * 15) ."";
					...
					if($rowcount > 0) {
						...
						while($row = $result->fetch_assoc()) {
							if($maxid < $row['id']) { $maxid = $row['id']; }
                                //echo $row['due_date'];
							    echo "<tr>";
                                echo "<td><a id='link_title' href='dashboardshow.php?table=". urlencode("scholarships") ."&id=". urlencode($row['id']) ."'>". $row['name'] ."</a></td>";
	                            echo "<td>". $row['due_date'] ."</td>";
							    //check admin
            					if(isset($_SESSION['login_user']) && $_SESSION['login_user'] != '') {
								echo "<td><a href='dashboardedit.php?table=". urlencode("scholarships") ."&id=". urlencode($row['id']) ."'>Modify</a></td>";
            		...
            		if(isset($_SESSION['login_user']) && $_SESSION['login_user'] != '') {
        				echo "<div><a href='dashboardadd.php?table=" .urlencode("scholarships")."'>Add to the database</a></div>";
					}
					if($page > 1) { echo "<a id='previous_page' href='dashboardcholarships.php?page=". urlencode($page - 1) .">Previous Page</a>";}
					if($page >= 1 && ($maxid % 15) == 0) { echo "<a id='next_page' href='dashboardscholarships.php?page=". urlencode($page + 1) .">Next Page</a>";}
					disconnect($conn);
		...
		<script type='text/javascript'>
			function delete_entry(id) {
				...
				if (answer) {
					window.location = 'dashboarddelete.php?table=scholarships&id=' + id;
				}
			}
